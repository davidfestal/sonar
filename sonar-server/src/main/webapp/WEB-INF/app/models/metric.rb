...
class Metric < ActiveRecord::Base
  ...
  def self.domains(translate=false)
    all.collect{|metric|
  ...
  def self.i18n_domain_for(to_translate)
    ...
    return localeMap[locale] if localeMap.has_key?(locale)
    
    i18n_key = 'domain.' + to_translate
    result = I18nHelper.translation(i18n_key, to_translate) 
    localeMap[locale] = result
    result
  end
  ...
  def short_name(translate=false)
  ...
  def self.by_domain(domain, translate=false)
